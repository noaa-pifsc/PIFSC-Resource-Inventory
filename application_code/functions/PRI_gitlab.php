	...
	class PRI_gitlab
	{
		...
		function replace_project_tags ($project_id, $gitlab_project_id, $existing_project = true)
		{
			...
			if ((!$existing_project) || (($existing_project) && ($rc = $this->oracle_db->query($SQL, $result, $dummy, $bind_array, OCI_NO_AUTO_COMMIT))))
			{
				...
				//request all Gitlab users accounts via v4 API:
				if ($content = curl_request("https://".GITLAB_HOST_NAME."/api/v4/projects/".$gitlab_project_id."/repository/tags?private_token=".GITLAB_API_KEY))
				{
					...
					if (count($data) == 0)
					{
						...
						echo $this->add_message("there are no project tags returned by the API request", 3);
