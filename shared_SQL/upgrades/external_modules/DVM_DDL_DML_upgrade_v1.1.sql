...
(select
(CASE WHEN RULE_INFO.column_name IS NULL THEN 'Missing Data QC View Indicator Field (DVM_ISS_TYPES.IND_FIELD_NAME)' ELSE NULL END) MISS_IND_FIELD_ERR_SOURCE,
(CASE WHEN RULE_INFO.column_name IS NULL THEN 'The Validation Rule ('||RULE_INFO.ISS_TYPE_NAME||') has an indicator field ('||RULE_INFO.IND_FIELD_NAME||') that does not exist in the corresponding Data QC View ('||RULE_INFO.OBJECT_NAME||'), the data QC view must be updated to define the indicator field or the indicator field name must be changed to match a data QC view field' ELSE NULL END) MISS_IND_FIELD_ERR_MSG,

(CASE WHEN RULE_INFO.COL_DATA_TYPE IS NOT NULL AND RULE_INFO.COL_DATA_TYPE NOT IN ('VARCHAR', 'VARCHAR2', 'CHAR') THEN 'Invalid Data Type for Data QC View Indicator Field (DVM_ISS_TYPES.IND_FIELD_NAME)' ELSE NULL END) INV_IND_FIELD_ERR_SOURCE,
(CASE WHEN RULE_INFO.COL_DATA_TYPE IS NOT NULL AND RULE_INFO.COL_DATA_TYPE NOT IN ('VARCHAR', 'VARCHAR2', 'CHAR') THEN 'The Validation Rule ('||RULE_INFO.ISS_TYPE_NAME||') has an indicator field ('||RULE_INFO.IND_FIELD_NAME||') that has an invalid data type in the corresponding Data QC View ('||RULE_INFO.OBJECT_NAME||'), the data QC view must be updated to define the indicator field as a VARCHAR, VARCHAR2, or CHAR data type' ELSE NULL END) INV_IND_FIELD_ERR_MSG

FROM

(select DVM_CRITERIA_V.ind_field_name,
DVM_CRITERIA_V.object_name,
DVM_CRITERIA_V.iss_type_name,
(select all_tab_cols.column_name from all_tab_cols where all_tab_cols.TABLE_NAME = DVM_CRITERIA_V.OBJECT_NAME AND
all_tab_cols.COLUMN_NAME = DVM_CRITERIA_V.IND_FIELD_NAME AND
all_tab_cols.owner = sys_context( 'userenv', 'current_schema' )) column_name,
(select all_tab_cols.DATA_TYPE from all_tab_cols where all_tab_cols.TABLE_NAME = DVM_CRITERIA_V.OBJECT_NAME AND
all_tab_cols.COLUMN_NAME = DVM_CRITERIA_V.IND_FIELD_NAME AND
all_tab_cols.owner = sys_context( 'userenv', 'current_schema' )) COL_DATA_TYPE

FROM DVM_CRITERIA_V) RULE_INFO
WHERE RULE_INFO.column_name IS NULL
OR COL_DATA_TYPE NOT IN ('VARCHAR', 'VARCHAR2', 'CHAR'))
...
(
    ...
    IN
    (
			(MISS_IND_FIELD_ERR_SOURCE, MISS_IND_FIELD_ERR_MSG),
			(INV_IND_FIELD_ERR_SOURCE, INV_IND_FIELD_ERR_MSG)
    )
)
...
COMMENT ON TABLE DVM_STD_QC_IND_FIELDS_V IS 'DVM Standard Indicator Field Data QC Object Query (Standard QC View)
...
COMMENT ON COLUMN DVM_STD_QC_IND_FIELDS_V.ERR_SOURCE IS 'The source of the DVM configuration error';
COMMENT ON COLUMN DVM_STD_QC_IND_FIELDS_V.ERR_MSG IS 'Standard error message to provide information to the data staff on what invalid DVM configurations exist to help ensure it functions properly';



CREATE OR REPLACE VIEW

DVM_STD_QC_DS_VIEWS_V AS
SELECT
'Data QC View (DVM_QC_OBJECTS)' ERR_SOURCE,
(CASE WHEN DS_INFO.QC_PK_COL_NAME IS NULL AND DS_INFO.DATA_STREAM_PK_FIELD IS NOT NULL THEN 'The Data QC View ('||DS_INFO.OBJECT_NAME||') does not contain the associated parent table''s ('||DS_INFO.DATA_STREAM_PAR_TABLE||') primary key field ('||DS_INFO.DATA_STREAM_PK_FIELD||'), update the Data QC View to add the parent table''s primary key field' ELSE NULL END)  ERR_MSG
FROM
(SELECT
DISTINCT
DVM_DATA_STREAMS_V.DATA_STREAM_PAR_TABLE,
DVM_DATA_STREAMS_V.DATA_STREAM_PK_FIELD,
DVM_CRITERIA_V.OBJECT_NAME,
--if the data stream PK field is valid and the column does not exist in the data QC view result set then report the error, do not report the error if the primary key field is not valid since it will be identified by a different configuration QC query
(SELECT column_name from all_tab_cols where owner = sys_context( 'userenv', 'current_schema' ) AND all_tab_cols.table_name = DVM_CRITERIA_V.OBJECT_NAME AND all_tab_cols.column_name = DVM_DATA_STREAMS_V.DATA_STREAM_PK_FIELD) QC_PK_COL_NAME
FROM

DVM_DATA_STREAMS_V
INNER JOIN DVM_CRITERIA_V ON DVM_DATA_STREAMS_V.DATA_STREAM_ID = DVM_CRITERIA_V.DATA_STREAM_ID) DS_INFO
WHERE DS_INFO.QC_PK_COL_NAME IS NULL AND DS_INFO.DATA_STREAM_PK_FIELD IS NOT NULL
ORDER BY
1, 2;

COMMENT ON COLUMN DVM_STD_QC_DS_VIEWS_V.ERR_SOURCE IS 'The source of the DVM configuration error';
COMMENT ON COLUMN DVM_STD_QC_DS_VIEWS_V.ERR_MSG IS 'Standard error message to provide information to the data staff on what invalid DVM configurations exist to help ensure it functions properly';


--redefine the QC view comment:
COMMENT ON TABLE DVM_STD_QC_DS_VIEWS_V IS 'DVM Standard Data Stream Parent Table Data QC View  Query (Standard QC View)
...
UNION ALL
SELECT ERR_SOURCE, ERR_MSG FROM DVM_STD_QC_IND_FIELDS_V
UNION ALL
SELECT ERR_SOURCE, ERR_MSG FROM DVM_STD_QC_DS_VIEWS_V
)
...
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Data Validation Module', '1.1', TO_DATE('07-AUG-20', 'DD-MON-YY'), 'Updated the DVM_PKG package to determine if each validation rule''s indicator field exists in the associated data QC view object''s result set fields, if not then a user defined exception is raised (-20234).  Updated the DVM_PKG package to raise an exception when a validation rule''s indicator filed in the corresponding data QC View has a non-character data type (-20235).  Updated the DVM_PKG package to raise an exception when a parent table''s primary key is not included in a Data QC View that is being evaluated (-20236).  Created a new DVM configuration QC view (DVM_STD_QC_IND_FIELDS_V) to identify each indicator field that does not have a matching result set field in the associated data QC view or do not have a character data type.  Created a new DVM configuration QC view (DVM_STD_QC_DS_VIEWS_V) to identify all Data QC Views that do not contain the primary key field of the associated parent table.  Updated the combined DVM configuration QC view (DVM_STD_QC_ALL_RPT_V) to include the new DVM_STD_QC_IND_FIELDS_V and DVM_STD_QC_DS_VIEWS_V views in the result set');
