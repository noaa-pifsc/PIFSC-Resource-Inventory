...
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Data Validation Module', '1.2', TO_DATE('08-SEP-20', 'DD-MON-YY'), 'Updated the DVM_PKG package to remove the return codes for all procedures (except for RETRIEVE_PARENT_ISSUE_REC_SP to indicate if the parent issue record exists or not) in favor of raising custom Oracle exceptions.  Implemented VALIDATE_PARENT_RECORD_RC_SP to provide the numeric return code and exception message parameters for use cases where an exception is not desirable (e.g. batch DVM executions).  Removed some unnecessary debug logging statements.');
