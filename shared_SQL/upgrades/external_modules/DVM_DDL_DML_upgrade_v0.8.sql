...
(CASE WHEN user_tables.table_name IS NOT NULL THEN 'Y' ELSE 'N' END) PAR_TABLE_EXISTS_YN,
(CASE WHEN USER_TAB_COLS.COLUMN_NAME IS NOT NULL THEN 'Y' ELSE 'N' END) PAR_TABLE_ENABLED_YN
from
dvm_data_streams left join
user_tables on
user_tables.table_name = DVM_DATA_STREAMS.DATA_STREAM_PAR_TABLE
left join
USER_TAB_COLS
ON USER_TAB_COLS.TABLE_NAME = DVM_DATA_STREAMS.DATA_STREAM_PAR_TABLE AND USER_TAB_COLS.COLUMN_NAME = 'PTA_ISS_ID' AND USER_TAB_COLS.DATA_TYPE = 'NUMBER'
left join
(
...
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Data Validation Module', '0.8', TO_DATE('20-JUL-20', 'DD-MON-YY'), 'Updated DVM_DATA_STREAMS_V view to include additional calculated columns to indicate if the associated parent table exists in the database and if the parent table is enabled (contains a PTA_ISS_ID numeric field), these values are used in the DVM_PKG package to validate the defined data streams.  Substantial updates were made to the DVM_PKG package to implement user defined exceptions for each error condition in the different procedures and functions.  Updated all of the header code comments for each procedure and function to ensure they are documented in addition to the updated inline code comments.  Implemented a partial rollback in the DVM packge to allow all record operations performed in the DVM package to be rolled back if there was a DVM package processing error.  Implemented standard database logging in each procedure/function to facilitate troubleshooting the package in different database instances/schemas.  Cleaned up unnecessary procedure variables that are no longer used.  Removed the explicit transaction commit when the procedure has successfully validated a record to allow the DVM package to be executed as part of a larger SQL transaction.  The DVM_PTA_ISSUES_V view was updated to switch the left joins to inner joins since additional records were being returned, the only left join is now with the associated issue resolution type (DVM_ISS_RES_TYPES).  Updated DVM_QC_MSG_MISS_FIELDS_V view to rename the MISSING_ISS_DESC_FIELDS to MISSING_ISS_TEMPL_FIELDS to be more accurate.');
