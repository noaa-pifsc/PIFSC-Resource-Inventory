...
--update the data model to require a QC object for each validation rule
ALTER TABLE DVM_ISS_TYPES
MODIFY (QC_OBJECT_ID NOT NULL);
...
alter view DVM_PTA_RULE_SETS_HIST_V compile;
...
alter view DVM_RULE_SETS_V compile;
...
(CASE WHEN PAR_TABLE_EXISTS_YN = 'Y' AND PAR_TABLE_ENABLED_YN = 'N' THEN 'Data Stream Parent Table Error (Not Enabled) - The Data Stream Code ('||DATA_STREAM_CODE||') has a parent table ('||DATA_STREAM_PAR_TABLE||') that has not been enabled in the DVM (this requires the parent table to have a NUMBER field named PTA_ISS_ID)' ELSE NULL END)
...
COMMENT ON TABLE DVM_STD_QC_DS_V IS 'DVM Standard Configuration QC Data Stream Parent Table Query (Standard QC View)
...
(CASE WHEN MISSING_ISS_TEMPL_FIELDS IS NOT NULL THEN 'Unmatched Template Placeholders (Issue Description) - The Validation Rule "'||ISS_TYPE_NAME||'" has an Issue Description Template that contains one or more unmatched placeholders ('||TO_CHAR(MISSING_ISS_TEMPL_FIELDS)||') that are not not found in the data QC view ('||OBJECT_NAME||'), either remove the placeholders from the template or update the data QC view to include the additional fields' ELSE NULL END) ISSUE_DESC_ERR_MSG,
...
(CASE WHEN MISSING_APP_LINK_FIELDS IS NOT NULL THEN 'Unmatched Template Placeholders (Application Link) - The Validation Rule "'||ISS_TYPE_NAME||'" has an Application Link Template that contains one or more unmatched placeholders ('||TO_CHAR(MISSING_APP_LINK_FIELDS)||') that are not not found in the data QC view ('||OBJECT_NAME||'), either remove the placeholders from the template or update the data QC view to include the additional fields' ELSE NULL END) APP_LINK_ERR_MSG
...
COMMENT ON TABLE DVM_STD_QC_ISS_TEMPL_V IS 'DVM Standard Configuration QC Issue Type Template Placeholders Query (Standard QC View)
...
select
--DATA_STREAM_CODE,
'Data QC View Object (DVM_QC_OBJECTS)' ERR_SOURCE,
ERR_MSG
FROM

(select
DVM_QC_OBJECTS.OBJECT_NAME,
CASE WHEN all_objects.object_name IS NULL THEN 'Data QC View Does Not Exist - The data QC View "'||DVM_QC_OBJECTS.OBJECT_NAME||'" does not exist, either remove the data QC object record or update the record with the correct data QC view name' ELSE NULL END MISS_VIEW_ERR_MSG,
CASE WHEN all_objects.object_name IS NOT NULL AND STATUS = 'INVALID' THEN 'Data QC View is Invalid - The data QC View "'||DVM_QC_OBJECTS.OBJECT_NAME||'" is invalid, the data QC view errors must be resolved before the DVM can be executed' ELSE NULL END INV_VIEW_ERR_MSG
...
COMMENT ON TABLE DVM_STD_QC_VIEW_V IS 'DVM Standard Configuration QC data QC Object Query (Standard QC View)
...
COMMENT ON TABLE DVM_STD_QC_ACT_RULE_SETS_V IS 'DVM Standard Configuration QC Active Validation Rules/Rule Sets Query (Standard QC View)
...
CASE WHEN count(*) = 0 THEN 'There are no Data Streams defined in the DVM, at least one Data Stream must be defined to execute the DVM' ELSE NULL END DATA_STREAM_ERR_MSG
FROM DVM_DATA_STREAMS
) DS_INFO,
(SELECT
CASE WHEN count(*) = 0 THEN 'Data Validation Rules (DVM_ISS_TYPES)' ELSE NULL END RULE_ERR_SRC,
CASE WHEN count(*) = 0 THEN 'There are no Validation Rules defined in the DVM, at least one Validation Rule must be defined to execute the DVM' ELSE NULL END RULE_ERR_MSG
from dvm_iss_types
) RULE_INFO)
...
COMMENT ON TABLE DVM_STD_QC_MISS_CONFIG_V IS 'DVM Standard Configuration QC Missing Data Streams/Validation Rules Query (Standard QC View)
...
--redefine the QC view comment:
COMMENT ON TABLE DVM_STD_QC_ALL_RPT_V IS 'DVM Standard Configuration QC Combined Report (Standard QC View)
...
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Data Validation Module', '1.0', TO_DATE('30-JUL-20', 'DD-MON-YY'), 'Modified the DVM_ISS_TYPES table to specify the Issue Severity (ISS_SEVERITY_ID) and QC View Object (QC_OBJECT_ID) foreign keys to be NOT NULL, recompiled dependent views and packages.  Updated the DVM Configuration QC views to modify the text that is reported to the user for consistency: DVM_STD_QC_DS_V, DVM_STD_QC_ISS_TEMPL_V, DVM_STD_QC_VIEW_V, DVM_STD_QC_ACT_RULE_SETS_V, DVM_STD_QC_MISS_CONFIG_V.  Updated the view comment on DVM_STD_QC_ALL_RPT_V to be consistent with the other standard DVM configuration QC views.  Updated the DVM_PKG to standardize the user defined exception messages for consistency and correct some user defined exception numbers that were listed incorrectly.  Also updated the QC_MISSING_QUERY_FIELDS_FN "OTHER" exception to split it into two separate exception codes depending on if this is an application link or an issue description error.');
