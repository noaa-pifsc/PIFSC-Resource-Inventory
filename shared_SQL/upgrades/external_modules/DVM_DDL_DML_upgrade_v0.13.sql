...
--all subqueries (so far the most efficient - cost of 222):
...
(SELECT (CASE WHEN count(*) = 1 AND SUM(CASE WHEN all_tab_cols.DATA_TYPE = 'NUMBER' THEN 1 ELSE 0 END) = 1 THEN max(cons_cols.COLUMN_NAME) ELSE NULL END) FROM all_cons_columns cons_cols
inner join all_CONSTRAINTS all_cons on 	all_cons.TABLE_NAME       = cons_cols.TABLE_NAME
	AND all_cons.CONSTRAINT_NAME  = cons_cols.CONSTRAINT_NAME
	AND all_cons.owner = cons_cols.owner
inner join all_tab_cols on
all_tab_cols.COLUMN_NAME = cons_cols.COLUMN_NAME
AND all_tab_cols.TABLE_NAME = cons_cols.TABLE_NAME
AND all_tab_cols.OWNER = cons_cols.OWNER
...
This query returns all data streams that are implemented in the data validation module.  Examples of data streams are RPL, eTunaLog, UL, FOT, LFSC.  This is used to filter error records based on the given context of the processing/validation.  This view also returns the PK field name for each of the data streams based on the value of DATA_STREAM_PAR_TABLE using the current schema''s data dictionary and determines if the parent table exists and if the parent table has been enabled in the DVM.  If the parent table does not have a primary key or has a compound primary key the DATA_STREAM_PK_FIELD field will be null';

--update terminology for data QC criteria view
COMMENT ON TABLE DVM_CRITERIA_V IS 'Data Validation QC Criteria (View)

This View returns all data QC Criteria (Issue Types) defined in the database and their associated QC Object, Issue Severity, and Issue Category.  This query is used to define all PTA Issue Types when a data stream is first entered into the database';
...
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Data Validation Module', '0.13', TO_DATE('28-JUL-20', 'DD-MON-YY'), 'Updated the DVM_DATA_STREAMS_V view to handle cases where there are compound primary keys and to ensure that the only way a DATA_STREAM_PK_FIELD value is returned is if there is a single, numeric primary key defined for the given parent table.  Updated the DVM_PKG to verify that each parent table has a single numeric primary key defined for it, this is a DVM requirement.  Updated the comment on the DVM_CRITERIA_V view to update terminology for data QC views');
