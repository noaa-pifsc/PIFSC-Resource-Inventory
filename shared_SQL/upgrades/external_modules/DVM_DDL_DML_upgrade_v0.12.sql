...
select
--DATA_STREAM_CODE,
'Data Stream (DVM_DATA_STREAMS)' ERR_SOURCE,
ERR_MSG
FROM
...
DATA_STREAM_CODE,
--PAR_TABLE_EXISTS_YN,
(CASE WHEN PAR_TABLE_EXISTS_YN = 'N' THEN 'Data Stream Parent Table Error (Does Not Exist) - The Data Stream Code ('||DATA_STREAM_CODE||') has a parent table ('||DATA_STREAM_PAR_TABLE||') that does not exist' ELSE NULL END)
TABLE_EXISTS_ERR_MSG,
--PAR_TABLE_ENABLED_YN,
--only report the enabled error if the parent table exists
(CASE WHEN PAR_TABLE_EXISTS_YN = 'Y' AND PAR_TABLE_ENABLED_YN = 'N' THEN 'Data Stream Parent Table Error (Not Enabled) - The Data Stream Code ('||DATA_STREAM_CODE||') has a parent table ('||DATA_STREAM_PAR_TABLE||') that has not been enabled in the DVM' ELSE NULL END)
TABLE_ENABLED_ERR_MSG,
--only report the primary key error if the parent table exists
(CASE WHEN PAR_TABLE_EXISTS_YN = 'Y' AND DATA_STREAM_PK_FIELD IS NULL THEN 'Data Stream Parent Table Error (Primary Key) - The Data Stream Code ('||DATA_STREAM_CODE||') has a parent table ('||DATA_STREAM_PAR_TABLE||') that does not have a single numeric primary key defined for it' ELSE NULL END) PRIMARY_KEY_ERR_MSG
...
COMMENT ON COLUMN DVM_STD_QC_DS_V.ERR_SOURCE IS 'The source of the DVM configuration error';
COMMENT ON COLUMN DVM_STD_QC_DS_V.ERR_MSG IS 'Standard error message to provide information to the data staff on what invalid DVM configurations exist to help ensure it functions properly';


COMMENT ON TABLE DVM_STD_QC_DS_V IS 'DVM Data Stream Configuration QC Query (Standard QC View)

This QC view identifies all instances of data streams with invalid values.  This view leverages the DVM_DATA_STREAMS_V view and uses the UNPIVOT function to split each identified issue into separate rows so they can be reported separately.  The view contains two columns that are intended for UNION with other DVM Configuration QC Queries to provide administrators the results of preliminary validation checks on the DVM configuration.';
...
ISS_TYPE_NAME,
--PAR_TABLE_EXISTS_YN,
(CASE WHEN MISSING_ISS_TEMPL_FIELDS IS NOT NULL THEN 'Issue Type Issue Description Template Placeholders (DVM_ISS_TYPES.ISS_TYPE_COMMENT_TEMPLATE)' ELSE NULL END) ISSUE_DESC_ERR_SOURCE,
(CASE WHEN MISSING_ISS_TEMPL_FIELDS IS NOT NULL THEN 'Unmatched Template Placeholders (Issue Description) - The Issue Type "'||ISS_TYPE_NAME||'" for the "'||OBJECT_NAME||'" QC view has an Issue Description Template that has one or more unmatched placeholders ('||TO_CHAR(MISSING_ISS_TEMPL_FIELDS)||'), either remove the placeholders or update the QC view to include the additional fields' ELSE NULL END) ISSUE_DESC_ERR_MSG,
(CASE WHEN MISSING_APP_LINK_FIELDS IS NOT NULL THEN 'Issue Type Application Link Template Placeholders (DVM_ISS_TYPES.APP_LINK_TEMPLATE)' ELSE NULL END) APP_LINK_ERR_SOURCE,
(CASE WHEN MISSING_APP_LINK_FIELDS IS NOT NULL THEN 'Unmatched Template Placeholders (Application Link) - The Issue Type "'||ISS_TYPE_NAME||'" for the "'||OBJECT_NAME||'" QC view has an Application Link Template that has one or more unmatched placeholders ('||TO_CHAR(MISSING_APP_LINK_FIELDS)||'), either remove the placeholders or update the QC view to include the additional fields' ELSE NULL END) APP_LINK_ERR_MSG

FROM
DVM_QC_MSG_MISS_FIELDS_V)
unpivot exclude nulls
(
    (ERR_SOURCE, ERR_MSG)
    FOR
    ...
    IN
    (
			(ISSUE_DESC_ERR_SOURCE, ISSUE_DESC_ERR_MSG) as 'ISSUE DESCRIPTION',
			(APP_LINK_ERR_SOURCE, APP_LINK_ERR_MSG) as 'APP LINK'
    )
);



COMMENT ON COLUMN DVM_STD_QC_ISS_TEMPL_V.ERR_SOURCE IS 'The source of the DVM configuration error';
COMMENT ON COLUMN DVM_STD_QC_ISS_TEMPL_V.ERR_MSG IS 'Standard error message to provide information to the data staff on what invalid DVM configurations exist to help ensure it functions properly';


COMMENT ON TABLE DVM_STD_QC_ISS_TEMPL_V IS 'DVM Issue Type Template Placeholders QC Query (Standard QC View)
...
select
--DATA_STREAM_CODE,
'QC View Object (DVM_QC_OBJECTS)' ERR_SOURCE,
ERR_MSG
FROM

(select
DVM_QC_OBJECTS.OBJECT_NAME,
CASE WHEN all_objects.object_name IS NULL THEN 'View Does Not Exist - The QC View Object "'||DVM_QC_OBJECTS.OBJECT_NAME||'" does not exist, either remove the QC object record or update the record with the correct view name' ELSE NULL END MISS_VIEW_ERR_MSG,
CASE WHEN all_objects.object_name IS NOT NULL AND STATUS = 'INVALID' THEN 'View is Invalid - The QC View Object "'||DVM_QC_OBJECTS.OBJECT_NAME||'" is invalid, the view errors must be resolved before the DVM can be executed' ELSE NULL END INV_VIEW_ERR_MSG

from
DVM_QC_OBJECTS
left join
all_objects on
DVM_QC_OBJECTS.OBJECT_NAME = all_objects.object_name
AND
all_objects.owner = sys_context( 'userenv', 'current_schema' )
AND
all_objects.object_type = 'VIEW')
unpivot exclude nulls
(
    ERR_MSG
    FOR
...
COMMENT ON COLUMN DVM_STD_QC_VIEW_V.ERR_SOURCE IS 'The source of the DVM configuration error';
COMMENT ON COLUMN DVM_STD_QC_VIEW_V.ERR_MSG IS 'Standard error message to provide information to the data staff on what invalid DVM configurations exist to help ensure it functions properly';


COMMENT ON TABLE DVM_STD_QC_VIEW_V IS 'DVM QC Object QC Query (Standard QC View)
...
--query for more than one active data stream rule set or no active rule sets:

create or replace view DVM_STD_QC_ACT_RULE_SETS_V
...
(SELECT
data_stream_code,
(CASE WHEN ACT_RULE_SET_COUNT > 1 THEN 'Active Validation Rule Set (DVM_RULE_SETS)' ELSE NULL END) ACT_RULE_SET_ERR_SRC,
(CASE WHEN ACT_RULE_SET_COUNT > 1 THEN 'Data Stream Validation Rule Set Error (multiple active rule sets) - The Data Stream ('||DS_INFO.DATA_STREAM_CODE||') has more than one ('||DS_INFO.ACT_RULE_SET_COUNT||') active validation rule set associated with it, only one rule set for each data stream should be active at any given time' ELSE NULL END) ACT_RULE_SET_ERR_MSG,
(CASE WHEN ACT_RULE_COUNT IS NULL THEN 'Inactive Validation Rules/QC Objects (DVM_ISS_TYPES/DVM_QC_OBJECTS)' ELSE NULL END) NO_ACT_RULES_ERR_SRC,
(CASE WHEN ACT_RULE_COUNT IS NULL THEN 'Data Stream Validation Rule Error (no active validation rules) - The Data Stream ('||DS_INFO.DATA_STREAM_CODE||') currently has no active validation rules, at least one active validation rule must be defined for the given data stream before it can be used to validate data' ELSE NULL END) NO_ACT_RULES_ERR_MSG
FROM
(select data_stream_code,
(select count(*) from dvm_rule_sets inner join dvm_data_streams DS ON dvm_rule_sets.data_stream_id = DS.DATA_STREAM_ID WHERE dvm_rule_sets.rule_set_active_yn = 'Y' AND dvm_rule_sets.DATA_STREAM_ID = DVM_DATA_STREAMS.DATA_STREAM_ID group by DS.DATA_STREAM_CODE, DS.DATA_STREAM_ID) ACT_RULE_SET_COUNT,
(select count(*) FROM DVM_CRITERIA_V where DVM_CRITERIA_V.DATA_STREAM_ID = DVM_DATA_STREAMS.DATA_STREAM_ID AND DVM_CRITERIA_V.ISS_TYPE_ACTIVE_YN = 'Y' AND DVM_CRITERIA_V.QC_OBJ_ACTIVE_YN = 'Y' group by DVM_CRITERIA_V.DATA_STREAM_ID, DVM_CRITERIA_V.DATA_STREAM_CODE) ACT_RULE_COUNT
FROM DVM_DATA_STREAMS order by data_stream_code) DS_INFO)
...
(
	...
	IN
	(
		(ACT_RULE_SET_ERR_MSG, ACT_RULE_SET_ERR_SRC),
		(NO_ACT_RULES_ERR_MSG, NO_ACT_RULES_ERR_SRC)
	)
)
;
...
COMMENT ON COLUMN DVM_STD_QC_ACT_RULE_SETS_V.ERR_SOURCE IS 'The source of the DVM configuration error';
COMMENT ON COLUMN DVM_STD_QC_ACT_RULE_SETS_V.ERR_MSG IS 'Standard error message to provide information to the data staff on what invalid DVM configurations exist to help ensure it functions properly';


COMMENT ON TABLE DVM_STD_QC_ACT_RULE_SETS_V IS 'DVM QC Active Validation Rules/Rule Sets Query (Standard QC View)
...
CREATE OR REPLACE VIEW DVM_STD_QC_MISS_CONFIG_V
AS
...
(select
DS_INFO.DATA_STREAM_ERR_SRC,
DS_INFO.DATA_STREAM_ERR_MSG,
RULE_INFO.RULE_ERR_SRC,
RULE_INFO.RULE_ERR_MSG
FROM
(SELECT CASE WHEN count(*) = 0 THEN 'Data Stream (DVM_DATA_STREAMS)' ELSE NULL END DATA_STREAM_ERR_SRC,
CASE WHEN count(*) = 0 THEN 'There are no DVM data streams defined in the database, at least one data stream must be defined to execute the DVM' ELSE NULL END DATA_STREAM_ERR_MSG
FROM DVM_DATA_STREAMS
) DS_INFO,
(SELECT
CASE WHEN count(*) = 0 THEN 'Data Validation Rules (DVM_ISS_TYPES)' ELSE NULL END RULE_ERR_SRC,
CASE WHEN count(*) = 0 THEN 'There are no DVM Validation Rules defined in the database, at least one validation rule must be defined to execute the DVM' ELSE NULL END RULE_ERR_MSG
from dvm_iss_types
) RULE_INFO)
...
(
	...
	IN
	(
		(DATA_STREAM_ERR_MSG, DATA_STREAM_ERR_SRC),
		(RULE_ERR_MSG, RULE_ERR_SRC)
	)
);


COMMENT ON COLUMN DVM_STD_QC_MISS_CONFIG_V.ERR_SOURCE IS 'The source of the DVM configuration error';
COMMENT ON COLUMN DVM_STD_QC_MISS_CONFIG_V.ERR_MSG IS 'Standard error message to provide information to the data staff on what invalid DVM configurations exist to help ensure it functions properly';


COMMENT ON TABLE DVM_STD_QC_MISS_CONFIG_V IS 'DVM QC Missing Data Streams/Validation Rules Query (Standard QC View)
...
CREATE OR REPLACE VIEW
DVM_STD_QC_ALL_RPT_V

AS
select ERR_SOURCE, ERR_MSG from

(select ERR_SOURCE, ERR_MSG from DVM_STD_QC_ISS_TEMPL_V
UNION ALL
select ERR_SOURCE, ERR_MSG from DVM_STD_QC_DS_V
UNION ALL
select ERR_SOURCE, ERR_MSG from DVM_STD_QC_VIEW_V
UNION ALL
select ERR_SOURCE, ERR_MSG FROM DVM_STD_QC_ACT_RULE_SETS_V
UNION ALL
SELECT ERR_SOURCE, ERR_MSG FROM DVM_STD_QC_MISS_CONFIG_V
)
ORDER BY ERR_SOURCE, ERR_MSG
;

COMMENT ON COLUMN DVM_STD_QC_ALL_RPT_V.ERR_SOURCE IS 'The source of the DVM configuration error';
COMMENT ON COLUMN DVM_STD_QC_ALL_RPT_V.ERR_MSG IS 'Standard error message to provide information to the data staff on what invalid DVM configurations exist to help ensure it functions properly';


COMMENT ON TABLE DVM_STD_QC_ALL_RPT_V IS 'DVM Standard All QC Report (Standard QC View)
...
--missing comment on DVM_PTA_RULE_SETS_HIST:
COMMENT ON TABLE DVM_PTA_RULE_SETS_HIST IS 'History tracking table for DVM_PTA_RULE_SETS implemented using the DSC.DSC_CRE_HIST_OBJS_PKG package';
...
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Data Validation Module', '0.12', TO_DATE('28-JUL-20', 'DD-MON-YY'), 'Developed DVM Configuration QC views to identify invalid DVM configurations in the given DVM instance.  The DVM_STD_QC_DS_V view was developed to identify issues with the data stream parent tables.  The DVM_STD_QC_ISS_TEMPL_V view was developed to identify validation rule issue description/application link template result set placeholders that are missing from the corresponding QC view objects.  The DVM_STD_QC_VIEW_V view identifies missing/invalid QC views.  The DVM_STD_QC_ACT_RULE_SETS_V view was developed to identify data streams with more than one active validation rule sets and data streams with no validation rules defined for it.  The DVM_STD_QC_MISS_CONFIG_V view was developed to identify when no data streams or no validation rules are defined in the DVM which indicates that the DVM instance is not configured.  The DVM_STD_QC_ALL_RPT_V view combines the results of all DVM configuration QC queries to allow the user to view all configuration problems in one query for convenience.  The DVM package was updated to correct an issue where the COMMA_DELIM_LIST_FN package function was reporting an incorrect user defined exception code.');
